...
|---|---|---|
| **R0：基盤立ち上げ** | `docker compose up` で開発環境が立ち上がる、CI が動く | モノレポ・DB・Redis・GitHub Actions・補完ツール一式（→ [ADR 0021](../../adr/0021-phase-0-tooling-discipline.md)） |
...
R0 は補完ツールの導入順序にロジックがあるため**直列**で進める。**根拠は [ADR 0021](../../adr/0021-phase-0-tooling-discipline.md) 参照**。技術選定の SSoT は [06-dev-workflow.md](../2-foundation/06-dev-workflow.md)。
