...
#### 3-3. **YAML（`.yaml` / `.yml`）** — ツール強制 / 慣習以外で選ぶ理由は無い

選ばない理由：
- インデント事故が発生しやすい（タブ vs スペース）
- 特殊文字を含む文字列のクォート要否が直感に反する（`workspace:*` は要クォート等）
- 型安全がゼロ
- TS / JSONC で書ける場合に YAML を選ぶメリットが薄い
...
新しいツールを導入する／既存ツールの形式を見直す
  ...
  └─ 自由選択
        │
        ├─ ツールが TS 型を export？ → TS（.ts）
        │
        ├─ そうでない＆純データ＆$schema あり？ → JSONC
        │
        └─ それ以外？ → JSONC（コメント書ければ）/ JSON（書けないなら）
                       ※YAML は選ばない（強制時以外）
```
