...
| メジャー更新（github-actions） | 許可（自動 PR 化） | SHA ピン止め（→ ADR 0026）の追従が滞ると前提が崩れる。breaking change は PR ごとに人間判断 |
...
| `github-actions` | **許可（自動 PR 化）** | GitHub 公式 Action はメジャー版が比較的短い間隔で続くことがある（actions/checkout v5 → v6 が数ヶ月）。メジャー版の変更内容は Node ランタイム要件の更新等が中心で、`with:` 引数の互換性に影響しない範囲に収まることが多い。`ignore` で止めると SHA ピン止めの追従が滞り、[ADR 0026](./0026-github-actions-sha-pinning.md) の前提（Dependabot で SHA を更新し続ける）が崩れる。**人間が PR ごとに breaking change の有無を確認・マージ判断する**運用とする |
