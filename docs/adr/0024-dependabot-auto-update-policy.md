...
|---|---|---|
| `github-actions` | **許可（自動 PR 化される）** | GitHub 公式 Action は短期間でメジャー版が切られる運用（v4 → v5 → v6 が数ヶ月ペース）。多くは Node ランタイム版の更新等で、`with:` 引数の互換性に影響しない範囲に収まることが多い。`ignore` で止めると SHA ピン止めの追従が滞り、ADR 0026 の前提（Dependabot で SHA を更新し続ける）が崩れる。**人間が PR ごとに breaking change の有無を確認・マージ判断する**運用とする |
