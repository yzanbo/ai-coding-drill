...
- GitHub Actions に至っては、後続 ADR で導入予定の SHA ピン止めと組み合わせると **メンテナが手で SHA を追従するのは現実的に不可能**（40 文字のハッシュを毎週手で書き換えるのは破綻する）
...
- **`commitlint.config.mjs` の `scope-enum` に `deps` と `deps-dev` を登録**（Dependabot が `include: scope` で生成する scope を許可）
...
| 候補 | 概要 | 採用しなかった理由 |
|---|---|---|
| A. 手動更新 | 必要時に `pnpm up` / Action 手書き更新 | 後続 ADR で導入する SHA ピン止めとの組み合わせが破綻（40 文字 SHA を手で追従不可）。脆弱性追従が属人化 |
...
- **後続 ADR の SHA ピン止めが実用可能になる**：人間が SHA を追従する負担なしに不変参照を維持できる
