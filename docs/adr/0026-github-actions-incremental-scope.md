...
一方で、[ADR 0021](./0021-phase-0-tooling-discipline.md) で導入済みの commitlint / Biome / 型チェックは、ローカルの lefthook と CI の両方で実行することにより初めて「PR レベルで規約違反を弾く」効果を持つ。これらは R0 時点で既に対象資産（コミット履歴・設定ファイル）が存在するため、CI 化の判断は今下せる。
